package com.xunlei.downloadprovider.database.greendao;

import android.database.Cursor;
import android.database.sqlite.SQLiteStatement;
import com.xunlei.downloadprovider.web.website.b.c;
import org.greenrobot.greendao.AbstractDao;
import org.greenrobot.greendao.database.Database;
import org.greenrobot.greendao.database.DatabaseStatement;
import org.greenrobot.greendao.internal.DaoConfig;

public class CollectWebsiteInfoDao extends AbstractDao<c, Long> {
    public static final String TABLENAME = "COLLECT_WEBSITE_INFO";

    protected final boolean isEntityUpdateable() {
        return true;
    }

    public CollectWebsiteInfoDao(DaoConfig daoConfig) {
        super(daoConfig);
    }

    public CollectWebsiteInfoDao(DaoConfig daoConfig, DaoSession daoSession) {
        super(daoConfig, daoSession);
    }

    public static void createTable(Database database, boolean z) {
        z = z ? "IF NOT EXISTS " : "";
        StringBuilder stringBuilder = new StringBuilder("CREATE TABLE ");
        stringBuilder.append(z);
        stringBuilder.append("\"COLLECT_WEBSITE_INFO\" (\"_id\" INTEGER PRIMARY KEY AUTOINCREMENT ,\"WEBSITE_URL\" TEXT,\"name\" TEXT,\"iconUrl\" TEXT,\"collectTime\" TEXT);");
        database.execSQL(stringBuilder.toString());
    }

    public static void dropTable(Database database, boolean z) {
        StringBuilder stringBuilder = new StringBuilder("DROP TABLE ");
        stringBuilder.append(z ? "IF EXISTS " : "");
        stringBuilder.append("\"COLLECT_WEBSITE_INFO\"");
        database.execSQL(stringBuilder.toString());
    }

    protected final void bindValues(DatabaseStatement databaseStatement, c cVar) {
        databaseStatement.clearBindings();
        Long l = cVar.a;
        if (l != null) {
            databaseStatement.bindLong(1, l.longValue());
        }
        String str = cVar.b;
        if (str != null) {
            databaseStatement.bindString(2, str);
        }
        str = cVar.c;
        if (str != null) {
            databaseStatement.bindString(3, str);
        }
        str = cVar.d;
        if (str != null) {
            databaseStatement.bindString(4, str);
        }
        cVar = cVar.e;
        if (cVar != null) {
            databaseStatement.bindString(5, cVar);
        }
    }

    protected final void bindValues(SQLiteStatement sQLiteStatement, c cVar) {
        sQLiteStatement.clearBindings();
        Long l = cVar.a;
        if (l != null) {
            sQLiteStatement.bindLong(1, l.longValue());
        }
        String str = cVar.b;
        if (str != null) {
            sQLiteStatement.bindString(2, str);
        }
        str = cVar.c;
        if (str != null) {
            sQLiteStatement.bindString(3, str);
        }
        str = cVar.d;
        if (str != null) {
            sQLiteStatement.bindString(4, str);
        }
        cVar = cVar.e;
        if (cVar != null) {
            sQLiteStatement.bindString(5, cVar);
        }
    }

    public Long readKey(Cursor cursor, int i) {
        i += 0;
        return cursor.isNull(i) ? null : Long.valueOf(cursor.getLong(i));
    }

    public c readEntity(Cursor cursor, int i) {
        int i2 = i + 0;
        Long valueOf = cursor.isNull(i2) ? null : Long.valueOf(cursor.getLong(i2));
        i2 = i + 1;
        String string = cursor.isNull(i2) ? null : cursor.getString(i2);
        i2 = i + 2;
        String string2 = cursor.isNull(i2) ? null : cursor.getString(i2);
        i2 = i + 3;
        i += 4;
        return new c(valueOf, string, string2, cursor.isNull(i2) ? null : cursor.getString(i2), cursor.isNull(i) ? null : cursor.getString(i));
    }

    public void readEntity(Cursor cursor, c cVar, int i) {
        int i2 = i + 0;
        String str = null;
        cVar.a = cursor.isNull(i2) ? null : Long.valueOf(cursor.getLong(i2));
        i2 = i + 1;
        cVar.b = cursor.isNull(i2) ? null : cursor.getString(i2);
        i2 = i + 2;
        cVar.c = cursor.isNull(i2) ? null : cursor.getString(i2);
        i2 = i + 3;
        cVar.d = cursor.isNull(i2) ? null : cursor.getString(i2);
        i += 4;
        if (!cursor.isNull(i)) {
            str = cursor.getString(i);
        }
        cVar.e = str;
    }

    protected final Long updateKeyAfterInsert(c cVar, long j) {
        cVar.a = Long.valueOf(j);
        return Long.valueOf(j);
    }

    public Long getKey(c cVar) {
        return cVar != null ? cVar.a : null;
    }

    public boolean hasKey(c cVar) {
        return cVar.a != null ? true : null;
    }
}
