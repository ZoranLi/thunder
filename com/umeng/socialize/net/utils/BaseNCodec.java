...
public abstract class BaseNCodec {
    ...
    public boolean isInAlphabet(byte[] bArr, boolean z) {
        int i = 0;
        while (i < bArr.length) {
            if (!isInAlphabet(bArr[i]) && (!z || (bArr[i] != (byte) 61 && !isWhiteSpace(bArr[i])))) {
                return false;
            }
            i++;
        }
    ...
    public long getEncodedLength(byte[] bArr) {
        ...
        return this.mLineLength > null ? length + ((((length + ((long) this.mLineLength)) - 1) / ((long) this.mLineLength)) * ((long) this.mChunkSeparatorLength)) : length;
    }
}
