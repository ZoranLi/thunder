...
class PageLog {
    ...
    public static void updateSession(Context context, String str, Long l, Long l2) {
        context = context.getSharedPreferences(FILE_SESSION, 0).edit();
        if (l.longValue() != 0) {
            context.putLong(LogBuilder.KEY_START_TIME, l.longValue());
        }
        context.putLong(LogBuilder.KEY_END_TIME, l2.longValue());
        context.commit();
    }
}
