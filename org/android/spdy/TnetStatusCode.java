...
public final class TnetStatusCode {
    ...
    public static final int getErrno(int i) {
        return i <= EASY_DEFAULT_ERROR ? 0 : i <= EASY_REASON_SLIGHTSSL_ERROR ? EASY_REASON_SLIGHTSSL_ERROR - i : i <= EASY_REASON_HANDSHAKE_ERROR ? EASY_REASON_HANDSHAKE_ERROR - i : i <= EASY_REASON_CONN_ERROR ? EASY_REASON_CONN_ERROR - i : i <= EASY_REASON_WRITE_ERROR ? EASY_REASON_WRITE_ERROR - i : i <= EASY_REASON_READ_ERROR ? EASY_REASON_READ_ERROR - i : 0;
    }
    ...
    public static final int getReqStage(int i) {
        i = getStatusCode(i);
        if (!(i == EASY_REASON_HANDSHAKE_ERROR || i == EASY_REASON_CONN_ERROR || i == EASY_REASON_IO_STOPED)) {
            if (i == EASY_SPDY_STREAM_IN_USE || i == EASY_SPDY_PROTOCOL_ERROR) {
                return -3;
            }
            if (!(i == EASY_REASON_CONN_TIMEOUT || i == EASY_REASON_CONN_NOT_EXISTS)) {
                if (i == 0) {
                    return 0;
                }
                switch (i) {
